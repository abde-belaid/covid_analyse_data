...
def clean_vaccination_data(df):
    """
    Cleans the vaccination data DataFrame by dropping rows with null dates and filling nulls in numeric columns with 0.
    """
...
def clean_testing_data(df):
    """
    Cleans the testing data DataFrame by dropping rows with null dates and filling nulls in numeric columns with 0.
    """
    # Drop rows where date is null
    df_cleaned = df.dropna(subset=["date"])
    
    # Fill nulls with 0 for numeric columns
    numeric_columns = ["total_tests", "new_tests"]
    df_cleaned = df_cleaned.fillna(0, subset=[c for c in numeric_columns if c in df_cleaned.columns])
    
    return df_cleaned

def clean_morality_data(df):
    """
    Cleans the morality data DataFrame by dropping rows with null dates and filling nulls in numeric columns with 0.
    """
    # Drop rows where date is null
    df_cleaned = df.dropna(subset=["date"])
    
    # Fill nulls with 0 for numeric columns
    numeric_columns = ["total_deaths", "new_deaths"]
    df_cleaned = df_cleaned.fillna(0, subset=[c for c in numeric_columns if c in df_cleaned.columns])
    ...
    if "people_fully_vaccinated" in df.columns:
        df_gold = (
            df.withColumn("year", year(col("date")))
              ...
              .groupBy("year", "month")
              .agg(spark_max("people_fully_vaccinated").alias("max_fully_vaccinated")) # Aggregate to find the maximum number of fully vaccinated people per month
              .orderBy("year", "month")
        )
    else:
        ...
        df_gold = (
            df.withColumn("year", year(col("date")))
              .withColumn("month", month(col("date")))
              .groupBy("year", "month")
              .count() # Count the number of records per month as a fallback
              .orderBy("year", "month")
        )
